...
| 33 | 08-09 | `d54554c` | The review attestation's digest split on the bare string `[semantic_review]`, which matched a **prose mention** in the file's own header — so it signed a prefix and reported itself stale on the run that wrote it. The test written for the fix was then vacuous: it compared two digests that the bug truncates *identically*, so disabling the anchor failed nothing | break-confirmation, on the test written to enforce break-confirmation |
