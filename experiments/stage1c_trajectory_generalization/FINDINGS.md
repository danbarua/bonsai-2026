...
`run_stage1c.py <seeds>` (defaults to all 10 trajectories in
`ALL_BASELINE_SEEDS`; seed=3000 is read from Stage 1B2's own results and
never re-run) builds and checkpoints each trajectory's 432 trials to
`results/stage1c_results_seed<N>.pkl`. `analyze_stage1c.py <seeds>` runs
