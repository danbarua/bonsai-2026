...
## MCP Server
`mighty-colab mcp` starts a stdio MCP (Model Context Protocol) server exposing this CLI's commands as tools for a *different* MCP-aware client (e.g. Codex Desktop) to call. It's an alternate entry point into the same commands documented here — not something to invoke from within this skill. If you already have shell access to `mighty-colab`, don't run `mighty-colab mcp` yourself: it blocks on stdio waiting for a client and will hang your session.
