...
Files may be gzip-compressed (.gz) or plain -- both are handled transparently.
"""
import gzip
import struct
import numpy as np
...
def load_mnist(data_dir: str = ".", gz: bool = False):
    """
    Load standard MNIST-format train/test split from IDX files in data_dir.
    ...
    Returns: (X_train, y_train, X_test, y_test)
      ...
      y_*: (N,) uint8 arrays (class labels, 0-9)
    """
    import os
    suffix = ".gz" if gz else ""
    X_train = load_idx_images(os.path.join(data_dir, f"train-images-idx3-ubyte{suffix}"))
    y_train = load_idx_labels(os.path.join(data_dir, f"train-labels-idx1-ubyte{suffix}"))
    X_test = load_idx_images(os.path.join(data_dir, f"t10k-images-idx3-ubyte{suffix}"))
    y_test = load_idx_labels(os.path.join(data_dir, f"t10k-labels-idx1-ubyte{suffix}"))
