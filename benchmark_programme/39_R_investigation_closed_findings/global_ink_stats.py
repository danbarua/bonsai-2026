...
def dct2d(img):
    """2D DCT-II, orthonormalized, via scipy's fast implementation."""
    return _dctn(img, norm='ortho')


def dct_low_order_10d(image_flat, side=28):
    """10 lowest-order 2D DCT coefficients (excluding the DC term),
    in zigzag order -- a standard, prespecified low-frequency image-domain
    control, dimensionality-matched to R."""
    img = image_flat.reshape(side, side)
    D = dct2d(img)
    zigzag_order = [(0,1),(1,0),(2,0),(1,1),(0,2),(0,3),(1,2),(2,1),(3,0),(4,0)]
    return np.array([D[i,j] for i,j in zigzag_order], dtype=np.float64)
