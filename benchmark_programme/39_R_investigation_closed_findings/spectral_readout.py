...
def build_spectral_basis_band(topology, band='low', n_eigenvectors=5):
    """Same active-node restriction and trivial-eigenvalue skipping as
    build_spectral_basis, but selects a different band of the spectrum:
    'low' (first 5 non-trivial, matches original R), 'mid' (5 centered
    in the middle of the non-trivial spectrum), 'high' (last 5)."""
    adj = np.abs(topology) > 0
    degrees = adj.sum(axis=1)
    active = np.where(degrees > 0)[0]
    n_active = len(active)
    W_active = np.abs(topology[np.ix_(active, active)])
    d_active = W_active.sum(axis=1)
    from scipy.sparse.csgraph import connected_components
    from scipy import sparse
    n_components, _ = connected_components(sparse.csr_matrix(W_active > 0), directed=False)
    ...
    n_nontrivial = n_active - n_components
    start_of_nontrivial = n_components
    if band == 'low':
        start = start_of_nontrivial
    elif band == 'mid':
        start = start_of_nontrivial + max(0, n_nontrivial // 2 - n_eigenvectors // 2)
    elif band == 'high':
        start = n_active - n_eigenvectors
    else:
        raise ValueError(band)
    end = min(start + n_eigenvectors, n_active)
    start = max(start, start_of_nontrivial)
    selected = eigvecs[:, start:end]
    return active, selected, eigvals[start:end]


def normalized_band_score(phases_flat, active_indices, eigenvectors, epsilon=1e-8):
    x_active = phases_flat[active_indices]
    projections = eigenvectors.T @ x_active
    raw_energy = np.sum(projections ** 2)
    active_support_energy = np.sum(x_active ** 2)
    return raw_energy / (active_support_energy + epsilon)


def build_lattice_graph_basis(active_indices, side=28, n_eigenvectors=5):
    """Non-oscillator graph control: a regular 4-connectivity pixel
    lattice graph (unit edge weights, no phase correlations anywhere),
    restricted to the same active-node set as the real topology for a
    fair comparison, then spectrally decomposed the same way as R."""
    active_set = set(active_indices.tolist())
    n = len(active_indices)
    index_map = {idx: i for i, idx in enumerate(active_indices)}
    W = np.zeros((n, n))
    ...
    from scipy.sparse.csgraph import connected_components
    from scipy import sparse
    n_components, _ = connected_components(sparse.csr_matrix(W > 0), directed=False)
    d = W.sum(axis=1)
    d_inv_sqrt = np.where(d > 0, 1.0/np.sqrt(d), 0.0)
    L_sym = np.eye(n) - (d_inv_sqrt[:,None] * W * d_inv_sqrt[None,:])
    eigvals, eigvecs = np.linalg.eigh(L_sym)
    start = n_components
    end = min(start + n_eigenvectors, n)
    return active_indices, eigvecs[:, start:end], eigvals[start:end]
